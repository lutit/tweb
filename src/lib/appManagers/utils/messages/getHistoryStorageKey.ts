...
export default function getHistoryStorageKey(options: RequestHistoryOptions & {type: HistoryStorage['type']}) {
  const {type, peerId, threadId, monoforumThreadId, virtualKey} = options;
  const filter = getSearchStorageFilterKey(options);
  return [type, peerId, filter, threadId, monoforumThreadId, virtualKey && `virtual:${virtualKey}`].filter(Boolean).join('_') as HistoryStorage['key'];
}
